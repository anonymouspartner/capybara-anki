...
--
-- NOT APPLIED ANYWHERE. This is a file on disk, committed for review, nothing more.
-- Per this repo's ground rules (README) and capybara-bot's CLAUDE.md, migrations
-- against the live Supabase project happen only on an explicit, in-the-moment
-- request from the maintainer — never as a side effect of writing code.
--
-- Open question this file does NOT resolve, deliberately: WHICH repo's migration
-- history actually carries this when it's applied. D4 puts this app on the same
-- Supabase project capybara-bot already uses, and capybara-bot/supabase/migrations/
-- is the versioned history already wired to deploy against that project (see its
-- CLAUDE.md). Whether this file's contents get applied by copying them into that
-- sequence, or by pointing a separate Supabase CLI config in this repo at the same
-- project ref, is a cross-repo call for the maintainer, not something to decide by
-- writing a file. Named and dated so it's easy to renumber into either home
-- verbatim, either way.
--
-- Written in the same idempotent style as capybara-bot's migrations (IF NOT EXISTS)
-- so re-running it is safe once it does get applied somewhere.
--
...
CREATE INDEX IF NOT EXISTS "card_state_due_idx" ON "public"."card_state" USING "btree" ("last_user_id", "due")
    WHERE (NOT "suspended");
...
CREATE INDEX IF NOT EXISTS "reviews_note_id_reviewed_at_idx" ON "public"."reviews" USING "btree" ("note_id", "reviewed_at");
...
CREATE TABLE IF NOT EXISTS "public"."scheduler_config" (
    ...
    CONSTRAINT "scheduler_config_pkey" PRIMARY KEY ("user_id"),
    CONSTRAINT "scheduler_config_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id")
);
