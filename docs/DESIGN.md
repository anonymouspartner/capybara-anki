...
| D21 | **A spelling card lives in the `Spelling` deck, not its note's deck** | Found by comparing the app's deck list against the AnkiDroid list it is supposed to mirror: Anki pins the `Capybara+` note type's second template to `Capybara::Spelling` (244 cards, template #1, confirmed in a real export), so a note's two cards sit in *different* decks. This app derived a card's deck from its note, which folded every spelling card into Ukrainian/English and made the Spelling deck vanish. Now `deckOfCard(noteDeck, cardKind)` decides, in one place, so the deck list, the queue and the daily counts cannot disagree. No column needed: a note has at most one spelling card, so the mapping is a pure function. Resolved 2026-09-17. |
| D22 | **Both people see the whole collection — D2's language scoping is withdrawn** | D2 assumed decks are disjoint by language and filtered every listing to the viewer's `users.learning_language`. Measured against the real collection that cost more than it bought: 199 English notes existed and the `uk` learner could not see the English deck, while the AnkiDroid collection they actually share showed it to him — which is how the gap was found. The bot's own help has always said to study "the sub-deck for the language you're learning, or the parent deck to drill both". The consequence is the one D2 itself named: `anki_card_state` is per card, so two people studying the same card share its schedule. `anki_reviews.user_id` still records who answered, so §4.3's replay can split a card's state per person if that ever matters. Resolved 2026-09-17. |
| D23 | **Pronunciation reference audio lives in Supabase Storage** | `transform.py` set every pronunciation note's `audio_url` to `None`, commenting that media "was deliberately excluded from the export". A real `Include media` export disproves that: 190 `capy_pron_*.mp3` files, one per note, referenced as `[sound:…]` from `ReferenceAudio`. With all 190 rows null the reviewer rendered no `<audio>` element at all, so a shadowing card had nothing to shadow. `migration/upload_pronunciation_audio.py` carries them into a public-read `pronunciation-audio` bucket and links them by `(lemma, language, kind)` — verified against the real export and the live table: **190 of 190 notes match**. Public-read because a plain `<audio src>` sends no Authorization header and a signed URL would expire while a card sat open. Resolved 2026-09-17. |
